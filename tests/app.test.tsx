...
describe('portfolio experience', () => {
  ...
  it('renders five cover-led cards without GitHub star counts', () => {
    const { container } = renderRoute()
    ...
    expect(container.querySelectorAll('.project-polaroid')).toHaveLength(5)
    ...
    expect(screen.getByRole('heading', { name: '佐佑动漫社 · 星际年度报告' })).toBeInTheDocument()
    expect(screen.getByRole('heading', { name: '佐佑动漫社 · 冒险者公会' })).toBeInTheDocument()
    ...
    expect(screen.getByText('北京市大学生创新创业大赛市级奖项')).toBeInTheDocument()
    expect(screen.getByText(/文本感知退化.*模型训练实验/)).toBeInTheDocument()
  ...
  it('switches the full interface to English', async () => {
    ...
    expect(screen.getAllByText(/01 repository$/)).toHaveLength(4)
